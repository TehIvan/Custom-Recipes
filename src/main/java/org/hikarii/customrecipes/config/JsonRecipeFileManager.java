...
public class JsonRecipeFileManager {
    ...
    public void saveRecipeJson(String recipeKey, Map<String, Object> recipeData) throws IOException {
        saveRecipeJson(recipeKey, recipeData, null);
    }

    public void saveRecipeJson(String recipeKey, Map<String, Object> recipeData, RecipeType type) throws IOException {
        File folder = getFolderForType(type);
        File jsonFile = new File(folder, recipeKey + ".json");
        try (Writer writer = new FileWriter(jsonFile)) {
            gson.toJson(recipeData, writer);
        }
        plugin.debug("Saved recipe to JSON: " + jsonFile.getPath());
    }

    public Map<String, Object> loadRecipeJson(String recipeKey) throws IOException {
        File jsonFile = findJsonFile(recipeKey);
        if (jsonFile == null) {
            return null;
        }
    ...
    private File findJsonFile(String recipeKey) {
        ...
        if (subfolders != null) {
            for (File subfolder : subfolders) {
                jsonFile = new File(subfolder, recipeKey + ".json");
                if (jsonFile.exists()) {
                    return jsonFile;
                }
            }
    ...
    public boolean deleteRecipe(String recipeKey) {
        ...
        if (jsonFile != null && jsonFile.exists()) {
            ...
            if (deleted) {
                plugin.debug("Deleted JSON file: " + jsonFile.getPath());
            }
            return deleted;
        }
        return false;
    ...
    public boolean jsonFileExists(String recipeKey) {
        return findJsonFile(recipeKey) != null;
    }

    public boolean jsonFileExists(String recipeKey, RecipeType type) {
        File folder = getFolderForType(type);
        return new File(folder, recipeKey + ".json").exists();
    }
}